  ...
  try {
    ...
    switch(source) {
      case 'usgs':
        ...
        break;
      case 'emsc':
        response = await fetch(`${EMSC_API_BASE}?starttime=${start}&endtime=${end}&minmag=${minMag}&format=geojson`);
        res.status(200).json(await response.json());
        break;
      case 'iris':
        response = await fetch(`${IRIS_API_BASE}?starttime=${start}&endtime=${end}&minmag=${minMag}&format=geojson`);
        res.status(200).json(await response.json());
        break;
      case 'geonet':
